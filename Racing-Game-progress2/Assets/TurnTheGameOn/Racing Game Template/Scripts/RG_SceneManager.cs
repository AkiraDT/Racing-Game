...
public class RG_SceneManager : MonoBehaviour {
	...
	RG_CarUserControl playerController;
	...
	public void SpawnVehicles(int vehicle){
		...
		gameTime = raceData.readyTime[raceData.raceNumber];
		readyTimer = true;
		countDown = true;
...
//		newVehicle.GetComponent<RG_CarUserControl>().enabled = false;
	...
	void Update () {
		...
		if (gameStarted) {
			...
			if (Input.GetKeyDown (inputData.pauseKey) || Input.GetKeyDown (inputData.pauseJoystick)) {
				PauseButton ();
			}
			...
			if(readyTimer && gameTime <= 0)	{
				...
				for (int i = 1; i < racerInfo.Length; i++){
					racerInfo[i].aIHelper.enabled = true;
		...
		if (wrongWay) {
			...
			if (wrongWayTimer >= raceData.wrongWayDelay) {
	...
	public void ChangeTarget(int racerNumber, int waypointNumber){
		if (Application.isPlaying && waypointNumber == racerInfo[racerNumber].nextWP + 1) {
			racerInfo[racerNumber].aIHelper.ResetTimer();
					...
					if (raceData.raceLaps[raceData.raceNumber] > racerInfo [racerNumber].lap) {
						...
						//	racerInfo[0].aIHelper.enabled = true;
							...
							if(racerInfo[0].finalStanding == 1){
								...
								if (PlayerPrefs.GetString ("Race" + raceData.raceNumber.ToString () + "Status") == "COMPLETE"  && !raceData.unlimitedRewards[raceData.raceNumber]) {
								...
								if (raceData.autoUnlockNextRace == RaceData.Switch.On && raceData.raceNumber != raceData.raceNames.Length - 1) {
								...
								PlayerPrefs.SetString ("Race" + raceData.raceNumber.ToString() + "Status", "COMPLETE");
							}
	...
	public void GenTimeSpanFromSeconds( double seconds ){
		...
		string      timeInterval = string.Format("{0:D2}:{1:D2}",  interval.Minutes, interval.Seconds);
		timeString = timeInterval + "." + interval.Ticks.ToString ().Substring (1, interval.Ticks.ToString().Length > 3 ? 3 : 0);
