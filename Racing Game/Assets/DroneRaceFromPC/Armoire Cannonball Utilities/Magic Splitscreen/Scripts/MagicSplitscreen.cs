...
public class MagicSplitscreen : MonoBehaviour
{
    ...
    /// </summary>
	private GameObject cameraTarget1;

    /// <summary>
    /// The point in space that camera 2 looks at
    /// </summary>
	private GameObject cameraTarget2;
    ...
    void Update()
    {
        ...
        this.distanceBetweenPlayers = this.centralPosition - this.MainPlayer.transform.position;
        ...
        if (this.IsSplitscreenOn)
        {
         ...
         //   this.cameraTarget1 = this.player1.transform.position + this.cameraDisplacement2d;
		//	this.cameraTarget2 = this.player2.transform.position - this.cameraDisplacement2d;
			...
			this.PositionSplitscreenMask(this.secondaryCamera, this.player2.transform.position, this.player2.transform.position + this.cameraDisplacement2d);
        ...
        else
        {
			...
			this.MoveCamera(this.primaryCamera, this.MainPlayer);
        }
    }
    #endregion
    ...
    private void SetCentralPosition()
    {
        ...
        if (this.player1)
        {
			this.centralPosition += this.player1.transform.position;
        }

        if (this.player2)
        {
			this.centralPosition += this.player2.transform.position;
        }
	...
	private void MoveCamera(Camera camera, GameObject targetPos)
    {
        //camera.transform.localRotation = this.cameraQuaternion;
		camera.transform.localRotation = targetPos.transform.localRotation;
        //camera.transform.position = targetPos - (camera.transform.forward * this.cameraDistance);
		camera.transform.position  = Vector3.SmoothDamp(camera.transform.position, targetPos.transform.TransformPoint(positionBehindDrone + new Vector3(0, yScrollValue, zScrollValue)), ref velocitiCameraFollow, cameraFollowPositionTime);

		//camera.transform.position = player + this.distanceBetweenPlayers;
    }

	private void MoveCamera2(Camera camera, GameObject targetPos)
	{
		//camera.transform.localRotation = this.cameraQuaternion;
		camera.transform.localRotation = targetPos.transform.localRotation;
		//camera.transform.position = targetPos - (camera.transform.forward * this.cameraDistance);
		Vector3 player = Vector3.SmoothDamp(camera.transform.position, targetPos.transform.TransformPoint(positionBehindDrone + new Vector3(0, yScrollValue, zScrollValue)), ref velocitiCameraFollow, cameraFollowPositionTime);
    ...
    private void StartSplitscreenCamera()
    {
        ...
        this.secondaryCamera.transform.position = this.primaryCamera.transform.position;
        this.secondaryCamera.transform.rotation = this.cameraTarget2.transform.localRotation;
