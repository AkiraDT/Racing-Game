...
public class Editor_RG_SceneManager : Editor {
	...
	public override void OnInspectorGUI(){
		...
		if (GUILayout.Button ("Wayponts", GUILayout.MaxWidth(Screen.width * 0.5f), GUILayout.MaxHeight(40) )) {
			waypoints = true;
			info = false;
		}
		...
		if (GUILayout.Button ("Info", GUILayout.MaxWidth(Screen.width * 0.5f), GUILayout.MaxHeight(40) )) {
			info = true;
			waypoints = false;
		}
		...
		if (waypoints) {
			...
			SerializedProperty showWaypointMesh = serializedObject.FindProperty ("showWaypointMesh");
			EditorGUI.BeginChangeCheck ();
			EditorGUILayout.PropertyField (showWaypointMesh, true, GUILayout.MaxWidth (Screen.width * 0.7f));
		...
		if(info){
			EditorGUILayout.HelpBox("Vehicle Number: " + rg_SceneManager.raceData.vehicleNumber.ToString()
				...
				+ "\nRace Laps: " + rg_SceneManager.raceData.raceLaps[rg_SceneManager.raceData.raceNumber].ToString()
