...
public class RG_IKDriver : MonoBehaviour {
	...
	void Start () {
		...
		if (gameMode == "SINGLE PLAYER") {
			if(syncVars != null)
				syncVars.singleLocalPlayer = true;			
		}
	...
	void Update(){
		...
		if (steeringWheel != null && Time.timeScale > 0) {
			Vector3 temp2;
			float rotationLimit;
			...
			float tempShake = Random.Range (0f,1.0f);
			float tempRPM;
			...
			if (syncVars != null) {
				...
				//tempRPM = wheelCollider.rpm;

			tempShake = tempShake * wheelShake * (tempRPM / 25);
