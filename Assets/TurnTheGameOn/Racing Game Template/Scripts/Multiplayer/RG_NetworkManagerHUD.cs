...
namespace UnityEngine.Networking{
	...
	public class RG_NetworkManagerHUD : NetworkBehaviour	{
		...
		void OnSceneLoaded(Scene scene, LoadSceneMode m){
			if (SceneManager.GetActiveScene ().name == "Garage" && PlayerPrefs.GetString ("Game Mode") == "MULTIPLAYER" && !awakeCall) {
		...
		//		lobbyHUDReference.hostCancelWindow.SetActive (true);
			} else if (SceneManager.GetActiveScene ().name != "Garage" && SceneManager.GetActiveScene ().name != "TEST" && PlayerPrefs.GetString ("Game Mode") == "MULTIPLAYER") {
