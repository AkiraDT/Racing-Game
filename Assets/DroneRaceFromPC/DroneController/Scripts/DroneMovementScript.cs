...
public class DroneMovementScript: MonoBehaviour{
	...
	void TrickRoll(){
		if(Input.GetKeyDown(rollLeft)){
			tiltAmountSideways += 360;
		}
		if(Input.GetKeyDown(rollRight)){
			tiltAmountSideways -= 360;
		}
	}
	...
	void ChangeCam(){
		if(Input.GetKeyDown(changeCam)){
			camScript.ChangePosCamera();
		}
	}

	void StartShootFire(){
		if(Input.GetKeyDown(shootFire)){
			droneObject.GetComponent<ParticleManager> ().ShotFire ();
		}if(Input.GetKeyUp(shootFire)){
			droneObject.GetComponent<ParticleManager> ().StopFire ();
		}
	}
	...
	void Update(){
		...
		if(mobile_turned_on == false && joystick_turned_on == false){
			I = (Input.GetKey(KeyCode.W)) ? true : false;
			K = (Input.GetKey(KeyCode.S)) ? true : false;
			J = (Input.GetKey(KeyCode.A)) ? true : false;
			L = (Input.GetKey(KeyCode.D)) ? true : false;

			W = (Input.GetKey(KeyCode.I)) ? true : false;
	...
	//	else if(Input.GetKey(KeyCode.K)){
		...
		if(X){
			movementForwardSpeed = turboForwardSpeed;
			//Debug.Log("Turbo");
		}else if(!X){
	...
	public float turboForwardSpeed = 750.0f;
	...
	public string right_analog_y = "Vertical_Right";
	public string turbo = "Turbo";
	public KeyCode downButton = KeyCode.JoystickButton4;
	public KeyCode upButton = KeyCode.JoystickButton5;
	public KeyCode rollLeft = KeyCode.JoystickButton13;
	public KeyCode rollRight = KeyCode.JoystickButton14;
	public KeyCode changeCam = KeyCode.JoystickButton3;
	...
	private float Vertical_I = 0;

	private bool W,S,A,D,I,J,K,L,X;
	private float Z;
